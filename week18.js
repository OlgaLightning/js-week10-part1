...
function makeThree() {
	...
	button3.addEventListener('click', function () {
		window.localStorage.setItem('newUser', ' name=John, age=25')
	});
}
...
function makeFour() {
	...
	button4.addEventListener('click', function () {
		window.localStorage.setItem('infoSaved', true)
	});
}
...
function makeFive() {
	...
	button5.addEventListener('click', function () {
		window.localStorage.setItem('missingInfo', null)
	});
}
...
const showEight = () => {
	...
	const objectEight = JSON.parse(arrayEightString);
...
const showSeven = () => {
	...
	const objectSeven = JSON.parse(arraySevenString);
...
const showSix = () => {
	const arraySixString = localStorage.getItem('objectSix');
	const objectSix = JSON.parse(arraySixString);
...
const removeLastElement = () => {
	if (numbers.length > 0) {
		const numbersToRemove = localStorage.getItem('task19');
		const removedNumbers = JSON.parse(numbersToRemove);
		removedNumbers.pop();
		localStorage.setItem('task19', JSON.stringify(removedNumbers));
...
const removeFromCart = () => {
	let cartItems = localStorage.getItem('cartItems');
	if (cartItems.lenght != ' ') {
		const removedProduct = JSON.parse(cartItems);
		removedProduct.pop();

		localStorage.setItem('cartItems', JSON.stringify(removedProduct));
		console.log('Последний товар удален из массива numbers и сохранен в Local Storage.');
	} else {
		console.log('Массив cartItem пуст.');
	}
};
...
const showCart = () => {
	...
	const cartItemsObject = JSON.parse(cartItemsString);

	if (cartItemsString) {
		const resultElement = document.querySelector('.practicum23');
		for (const key in cartItemsObject) {
			const value = cartItemsObject[key];
			resultElement.innerHTML += `${value}, `
		}
	} else {
...
const updateCartCount = () => {
	...
	if (cartItemsString && cartItemsString.trim() !== 0) {
		const cartItemsObject = JSON.parse(cartItemsString);
		const cartItemCount = cartItemsObject.length;
		const resultElement = document.querySelector('.practicum24');
		resultElement.textContent = `Количество товаров в корзине ${cartItemCount}`;
	} else {
